...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL)
{
    ...
    $t->db->select($discount_filter_data . '(select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type, p.is_prices_inclusive_tax, p.type ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids,p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.description,p.total_allowed_quantity,p.deliverable_type,p.is_attachment_required,p.product_identity,p.deliverable_zipcodes,p.minimum_order_quantity,p.quantity_step_size,p.cod_allowed,p.row_order,p.rating,p.no_of_ratings,p.image,p.is_returnable,p.is_cancelable,p.cancelable_till,p.indicator,p.other_images, p.video_type, p.video, p.tags, p.warranty_period, p.guarantee_period, p.made_in,p.availability,c.name as category_name,tax.percentage as tax_percentage,tax.id as tax_id ')->join(" categories c", "p.category_id=c.id ", 'LEFT')
                ...
                if ($final_total >= intval($promo_code[0]['minimum_order_amount'])) {
                    ...
                    if ($promo_code[0]['repeat_usage'] == 1 && ($promo_code[0]['user_promo_usage_counter'] <= $promo_code[0]['no_of_repeat_usage'])) {
                        ...
                        if (intval($promo_code[0]['user_promo_usage_counter']) <= intval($promo_code[0]['no_of_repeat_usage']) || $recalculation == true) {
                            ...
                            if ($promo_code[0]['discount_type'] == 'percentage') {
                                $promo_code_discount =  (isset($promo_code[0]['is_cashback']) && $promo_code[0]['is_cashback'] == 0) ? floatval($final_total  * $promo_code[0]['discount'] / 100) : 0;
                            } else {
                                $promo_code_discount = (isset($promo_code[0]['is_cashback']) && $promo_code[0]['is_cashback'] == 0) ? floatval($final_total - $promo_code[0]['discount']) : 0;
                            }
                            if ($promo_code_discount <= $promo_code[0]['max_discount_amount']) {
                                $total = (isset($promo_code[0]['is_cashback']) && $promo_code[0]['is_cashback'] == 0) ? floatval($final_total) - $promo_code_discount : floatval($final_total);
                            } else {
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null)
{
    ...
    $search_res = $t->db->select(' o.*, u.username,u.country_code,p.name,a.name as user_name,a.mobile as recipient_contact ')->join(' `users` u', 'u.id= o.user_id', 'left')->join(' `order_items` oi', 'o.id= oi.order_id', 'left')->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')->join('addresses a', 'a.id=o.address_id', 'left')
        ->join('products p', 'pv.product_id=p.id', 'left');
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,p.cancelable_till,p.is_returnable,p.image,p.name,
        p.rating as product_rating,p.type, 
...
function process_refund($id, $status, $type = 'order_items')
{
    ...
    if ($type == 'order_items') {
        ...
        */
        ...
        /* send_notification if applicable */
            ...
            if ($wallet_refund == true) {
                ...
                if ($new_final_total >= $promo_discount) {
                    if (trim(strtolower($payment_method)) != 'cod' && $payment_method != 'Bank Transfer') {
                        ...
                        // echo "here $total - $final_total - $total_payable - $returnable_amount - $wallet_balance ";
