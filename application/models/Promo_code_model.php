...
class Promo_code_model extends CI_Model
{
    ...
    public function get_promo_code_list($offset = 0, $limit = 10, $sort = 'id', $order = 'ASC')
    {
        ...
        $search_res = $this->db->select(' p.`id` as id , p.`promo_code`, p.`image` , p.`message` , p.`start_date` , p.`end_date`, p.`discount` , p.`repeat_usage` ,p.`minimum_order_amount` ,p.`no_of_users` ,p.`discount_type` , p.`max_discount_amount`, p.`no_of_repeat_usage` , p.`status` , p.`is_cashback` ');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
    ...
    public function get_promo_codes($limit = "", $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL)
    {
        ...
        $where = "(CURDATE() between start_date AND end_date) and status = 1 and is_cashback = 0";
        $count_res->where($where);
        ...
        $search_res = $this->db->select(' p.`id` as id ,datediff(end_date, start_date ) as remaining_days, p.`promo_code`, p.`image` , p.`message` , p.`start_date` , p.`end_date`, p.`discount` , p.`repeat_usage` ,p.`minimum_order_amount` ,p.`no_of_users` ,p.`discount_type` , p.`max_discount_amount`, p.`no_of_repeat_usage` , p.`status` ,p.`is_cashback`');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
            $search_res->or_like($multipleWhere);
        }
        $where = "(CURDATE() between start_date AND end_date) and status=1  and is_cashback = 0";
    ...
    function settle_cashback_discount()
    {
        ...
        $where = "o.active_status='delivered' AND o.promo_code != '' AND o.promo_discount <= 0 HAVING date = '" . $date . "'";

        $data = $this->db->select("o.id,o.date_added,o.total,o.final_total,o.promo_code,o.user_id,DATE_ADD(date_format(o.date_added,'%Y-%m-%d'), INTERVAL " . $settings['max_product_return_days'] . " DAY) as date ")
            ->where($where)
        ...
        if (!empty($data)) {
            foreach ($data as $row) {
                ...
                $response = update_wallet_balance('credit', $user_id, $res['data'][0]['final_discount'], 'Discounted Amount Credited for Order ID  : ' . $row['id']);
            ...
            if ($wallet_updated == true) {
                ...
                foreach ($user_ids as $user) {
                    ...
                    send_mail($user_res[0]['email'], 'Discounted Amount Credited', 'Discounted Amount Credited, which orders are delivered. Please take note of it! Regards ' . $app_name . '');
                    $fcm_ids = array();
                    if (!empty($user_res[0]['fcm_id'])) {
                        $fcmMsg = array(
                            'title' => "Discounted Amount Credited",
                            'body' => 'Hello Dear ' . $user_res[0]['username'] . ' Discounted Amount Credited, which orders are delivered. Please take note of it! Regards ' . $app_name . '',
                            'type' => "Discounted"
                        );
